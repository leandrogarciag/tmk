document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (!btnCargue) {
    console.error("Botón de carga no encontrado en el DOM");
    return;
  }

  btnCargue.addEventListener('click', async (e) => {
    console.log("Estoy dando click al botón de envío");
    await enviarDatosTabla();
  });

  async function enviarDatosTabla() {
    const registros = [];
    document.querySelectorAll('#page-length-option tbody tr').forEach(row => {
      const documento = row.cells[0].textContent;
      const min = row.cells[1].textContent;
      registros.push({ documento, min });
    });

    const response = await fetch('/mensajeria/guardarRegistrosTMK', {
      method: 'POST',
      headers: {
  ...
  // Nueva función para enviar los datos a la tabla tbl_registros_tmk
