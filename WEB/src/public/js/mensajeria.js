  ...
  if (plantillasLocal != null) {
    for (let i = 0; i < plantillasLocal.length; i++) {
      ...
      <div class="collection card" style="border-radius: 5px;">
        <a  class="collection-item sectionPlantilla modal-close" style="color:#333333;cursor:pointer;"><strong>${plantillasLocal[i].PLA_CNOMBRE}:</strong> ${plantillasLocal[i].PLA_CCONTENIDO}</a>
      </div>
      `;
    }
    ...
    for (let i = 0; i < sectionPlantilla.length; i++) {
      ...
      sectionPlantilla[i].addEventListener('click', function () {
        ...
        const regex = /<\/strong>\s*(.*)/;
        const match = regex.exec(htmlContent);

        if (match && match[1]) {
          inputMessage.value = match[1].trim();
        } else {
          inputMessage.value = '';
        }
