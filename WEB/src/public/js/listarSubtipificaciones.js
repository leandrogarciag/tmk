document.addEventListener('DOMContentLoaded', () => {
  ...
  const eliminar = () => {
    document.querySelectorAll("a#borrar").forEach((botonBorrar) => {
      botonBorrar.addEventListener("click", function (e) {
        let ideliminar = this.getAttribute("eliminarSubtipificacion");
        ...
        Swal.fire({
          ...
          if (result.isConfirmed) {
            ...
            getData(`/subtipificaciones/eliminarSubtipificacion/${ideliminar}`).then(res => {
              location.reload();
            })
          }
        });
  ...
  $('#tuTablaSubtipificaciones').DataTable({
