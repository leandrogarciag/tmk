  ...
  document.getElementById("Buscar").addEventListener("click", async function (e) {
    ...
    if (reporteInteracciones) {
      if (reporteInteracciones.length > 0) {
        ...
        var table = $("#page-length-option").DataTable({
          data: reporteInteracciones,
          columns: [{ data: "PKGES_CODIGO" }, { data: "agente" }, { data: "cliente" }, { data: "tipificacion" }, { data: "radicado" }, { data: "observaciones" }, { data: "fechaInicio" }, { data: "fechaAsignacion" }, { data: "fechaFin" }, { data: "tiempo" }],
          responsive: true,
          iDisplayLength: 20,
          lengthMenu: [
            [20, 40, 60, 80, 100, -1],
