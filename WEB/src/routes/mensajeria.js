...
const app = express();
const cronJob = require('./reporteCronJob');
...
router.get('/masivo/cargar-excel', async (req, res) => {
  ...
  //res.render('auth/err404', { title: 'Página no encontrada' })
});
...
router.post('/masivo/guarde-local-excel', async (req, res) => {
  ...
  try {
    excel.mv(rutaArchivo, async (err) => {
      ...
      chmodSync(rutaArchivo, 'a+rwx');

      getDataExcel({ rutaArchivo, req })
        .then((registros) => {
          res.status(200).render('app/masivo/vistaPrevia', { title: 'Cargar Excel', registros, nombreExcel: excel.name, rutaArchivo });
        })
        .catch(reject => {
...
router.post('/guardarRegistrosTMK', async (req, res) => {
  try {
    const { registros } = req.body;
    const sqlInsert = `INSERT INTO ${DB}.tbl_registros_tmk (documento, min) VALUES ?`;
    const values = registros.map(reg => [reg.documento, reg.min]);
...
async function getDataExcel({ rutaArchivo, req }) {
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      const selectedHoja1 = workbook.getWorksheet(1);
      const cantFilas = selectedHoja1.actualRowCount;

      for (let i = 2; i <= cantFilas; i++) {
        const documento = selectedHoja1.getRow(i).getCell(1).text.trim();
        const min = selectedHoja1.getRow(i).getCell(2).text.trim();

        if (documento.length > 13 || !/^[0-9]+$/.test(documento)) {
          return reject(`El número de documento en la celda A${i} no es válido`);
        }

        if (min.length > 13 || !/^[0-9]+$/.test(min)) {
          return reject(`El número MIN en la celda B${i} no es válido`);
        }

        registros.add(JSON.stringify({ documento, min }));
      }

      const registrosUnicos = Array.from(registros).map(registro => JSON.parse(registro));

      // Verificar los registros en la base de datos
      const sqlSelect = `SELECT documento, min FROM ${DB}.tbl_registros_tmk`;
      const [registrosExistentes] = await db.promise().query(sqlSelect);

      const setExistentes = new Set(registrosExistentes.map(reg => JSON.stringify(reg)));
      const registrosFiltrados = registrosUnicos.filter(reg => !setExistentes.has(JSON.stringify(reg)));

      if (registrosFiltrados.length === 0) {
        return reject('Los registros seleccionados ya existen en la base de datos');
      }

      resolve(registrosFiltrados);
    } catch (err) {
      reject(`Error en el archivo Excel: ${err.message}`);
    }
  });
}
...
router.get('/reportes/getRegistrosTMK', async (req, res) => {
  try {
    const sqlSelect = `SELECT documento, min FROM ${DB}.tbl_registros_tmk`;
